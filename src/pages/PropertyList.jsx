    ...
    <section ref={sectionRef} className="max-w-7xl mx-auto px-4 py-24">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-10">
        ...
        >
          ...
          {[
            { title: 'Property Type', items: ['Apartment', 'House', 'Duplex', 'Bungalow', 'Studio'] },
            { title: 'Key Features', items: ['Swimming Pool', 'Balcony', 'Parking', 'Smart Home'] },
            { title: 'Listing', items: ['For Sale', 'For Rent', 'Short Let', 'Luxury', 'Newly Built'] }
          ].map(({ title, items }) => (
            <div key={title} className="bg-white p-6 rounded-xl shadow-sm border border-[#e5e5e5]">
              ...
              <div className="space-y-4 text-sm">
                {items.map((label, i) => (
                  <label key={i} className="flex items-center group cursor-pointer">
                    ...
                    <span className="text-[#0a0f1c]/70 group-hover:text-[#d4af37] transition-colors">{label}</span>
                  </label>
                ))}
              </div>
            </div>
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-[#e5e5e5]">
            ...
            <div className="space-y-4">
              <div className="flex gap-3">
                <input type="number" placeholder="Min" className="w-1/2 px-4 py-2.5 border border-[#e5e5e5] rounded-lg text-sm focus:border-[#d4af37] transition-colors" />
                <input type="number" placeholder="Max" className="w-1/2 px-4 py-2.5 border border-[#e5e5e5] rounded-lg text-sm focus:border-[#d4af37] transition-colors" />
              </div>
              ...
              <div className="p-6 bg-white">
                ...
                <div className="flex justify-between text-sm text-gray-600 mb-4">
                  <div className="flex items-center gap-1">
                    <span className="text-[#d4af37]">📐</span>{property.sqft} sqft
                  </div>
                  <div className="flex items-center gap-1">
                    <span className="text-[#d4af37]">🛏️</span>{property.bed} Beds
                  </div>
                  <div className="flex items-center gap-1">
                    <span className="text-[#d4af37]">🛁</span>{property.bath} Baths
                  </div>
                </div>
                <div className="flex justify-between items-center">
                  <div className="text-xl font-serif font-bold text-[#d4af37]">{property.price}</div>
